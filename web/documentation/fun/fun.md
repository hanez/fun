...
<pre>fun /usr/share/fun/examples/hello.fun</pre>

Running from a build tree (not installed):

<pre>FUN_LIB_DIR=./lib /path/to/build_dir/fun examples/hello.fun</pre>

Passing arguments to scripts (arguments after `--` are forwarded to the script environment):
<pre>fun myscript.fun -- arg1 arg2</pre>
...
./tmp/stage/usr/bin/fun ./examples/hello.fun</pre>

## See also
- [../cli/](../cli/) — concise CLI reference (synopsis/options/exit codes)
- [../funstx/](../funstx/) — syntax checker for `.fun` files with optional `--fix`
- [../examples/](../examples/) — how to run the bundled examples
- [../includes/](../includes) — include paths, namespacing, and `FUN_LIB_DIR`
